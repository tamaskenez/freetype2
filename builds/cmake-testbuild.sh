...
build_opts="-DWITH_ZLIB=0 -DWITH_BZip2=0 -DWITH_PNG=0 -DWITH_HarfBuzz=0 $bsl"
build_opts="$build_opts -DCMAKE_INSTALL_PREFIX=$tmpdir/out"
(set -x; cmake -H$ftdir -B$tmpdir/ftb -DCMAKE_BUILD_TYPE=Debug $build_opts)
(set -x; cmake --build $tmpdir/ftb --config Debug --target install)
(set -x; cmake $tmpdir/ftb -DCMAKE_BUILD_TYPE=Release)
(set -x; cmake --build $tmpdir/ftb --config Release --target install --clean-first)
...
mkdir -p $tmpdir/tb
cd $tmpdir/tb
export LD_LIBRARY_PATH=$tmpdir/out/lib:$LD_LIBRARY_PATH
export DYLD_LIBRARY_PATH=$tmpdir/out/lib:$DYLD_LIBRARY_PATH
(set -x; cmake $tmpdir -DCMAKE_BUILD_TYPE=Debug -DCMAKE_PREFIX_PATH=$tmpdir/out)
(set -x; cmake --build . --config Debug)
(set -x; ctest -V -C Debug)
(set -x; cmake . -DCMAKE_BUILD_TYPE=Release)
(set -x; cmake --build . --config Release --clean-first)
